...
| `status` | string | 用户状态：`active`、`pending`、`ban`；注册用户初始为 `pending`，管理员审核通过后改为 `active` |
...
| `allowCustomChannel` | bool | 是否允许已登录普通用户配置浏览器本地自定义渠道；默认关闭，用户 API Key 不写入后端数据库 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `protocol` | string | 协议，支持 `openai`、`gemini`、`fpbrowser2api`、`zerofall`；后三者在界面显示为 `Gemini 原生图片`、`zpika-veo-omni-flash`、`zpika-omni-flash` |
| `name` | string | 渠道名称 |
