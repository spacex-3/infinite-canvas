...
Allow an administrator to decide whether signed-in ordinary users may configure one browser-local AI channel with their own protocol, Base URL, API Key, and model list. Display `fpbrowser2api` as `zpika-veo-omni-flash` and `zerofall` as `zpika-omni-flash` without changing stored protocol values or backend routing.
...
- Add a browser-local `protocol` field with `openai`, `fpbrowser2api`, and `zerofall` values. The UI displays the last two values as `zpika-veo-omni-flash` and `zpika-omni-flash`; local image/video requests use the selected protocol, while cloud mode continues to route by the administrator's channel and known model names remain automatic fallbacks.
...
- API tests verify local `zerofall`/`fpbrowser2api` selections route aliases to their JSON video endpoints without allowing saved local protocol state to override cloud mode.
- UI source tests verify both zpika display labels, the public switch, user protocol field, unselected fetched-model options, tag-based model input, and removal of the ordinary-user-only-cloud restriction.
