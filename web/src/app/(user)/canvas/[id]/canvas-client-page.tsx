...
import { defaultConfig, isModelAvailableForCapability, type AiConfig, useConfigStore, useEffectiveConfig } from "@/stores/use-config-store";
...
import { getDataUrlByteSize, mapWithConcurrency, readImageMeta } from "@/lib/image-utils";
...
function InfiniteCanvasPage() {
    ...
    const createConnectedNode = useCallback(
        (type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Config | CanvasNodeType.Video | CanvasNodeType.Audio, pending: PendingConnectionCreate) => {
            const metadata = type === CanvasNodeType.Config ? { model: effectiveConfig.imageModel || effectiveConfig.model, size: effectiveConfig.size, videoSize: effectiveConfig.videoSize, count: getGenerationCount(effectiveConfig.canvasImageCount || effectiveConfig.count) } : undefined;
        ...
        [effectiveConfig.canvasImageCount, effectiveConfig.count, effectiveConfig.imageModel, effectiveConfig.model, effectiveConfig.size, effectiveConfig.videoSize, message, setConnecting],
    );
    ...
    const batchChildCountById = useMemo(() => {
        ...
        nodes.forEach((node) => {
            const batchCount = getImageBatchDisplayCount(node);
            if (batchCount) map.set(node.id, batchCount);
        });
        return map;
    }, [nodes]);
    ...
    const createNode = useCallback(
        ...
        [effectiveConfig.canvasImageCount, effectiveConfig.count, effectiveConfig.imageModel, effectiveConfig.model, effectiveConfig.size, effectiveConfig.videoSize, getCanvasCenter],
    );
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`朋克画布 ${useCanvasStore.getState().projects.length + 1}`);
    ...
    const handleGenerateNode = useCallback(
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            const updateGenerationProgress = (targetId: string, progress: number) => {
                setNodes((prev) => prev.map((node) => (node.id === targetId && node.metadata?.status === NODE_STATUS_LOADING ? { ...node, metadata: { ...node.metadata, progress } } : node)));
            };
            if (markSourceStatus) setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, prompt: statusPrompt, status: NODE_STATUS_LOADING, progress: undefined, errorDetails: undefined } } : node)));
            ...
            try {
                if (mode === "image") {
                    ...
                    const childNodes: CanvasNodeData[] = childIds.map((id, index) => ({
                        ...
                        width: imageConfig.width,
                        height: imageConfig.height,
                        metadata: { prompt: effectivePrompt, status: NODE_STATUS_LOADING, progress: undefined, batchRootId: count > 1 ? rootId : undefined, ...generationMetadata },
                    }));
                        ...
                        ...prev.map((node) =>
                            node.id === nodeId
                                ? isConfigNode
                                    ? {
                                          ...node,
                                          metadata: { ...node.metadata, prompt: effectivePrompt, status: NODE_STATUS_LOADING, progress: undefined, errorDetails: undefined },
                                      }
                                    : isEmptyImageNode
                                      ? {
                                            ...node,
                                      ...
                                      : isImageNode
                                        ? {
                                              ...node,
                                              metadata: { ...node.metadata, status: NODE_STATUS_SUCCESS, progress: undefined, errorDetails: undefined },
                                          }
                                        : {
                                              ...node,
                                              ...
                                              metadata: { ...node.metadata, content: prompt, prompt, status: NODE_STATUS_SUCCESS, progress: undefined, fontSize: 14, errorDetails: undefined },
                                          }
                                : node,
                        ),
                    ...
                    await mapWithConcurrency(targetIds, IMAGE_BATCH_CONCURRENCY, async (targetId) => {
                        try {
                            const image = referenceImages.length
                                ? await requestEdit({ ...generationConfig, count: "1" }, effectivePrompt, referenceImages, undefined, (progress) => updateGenerationProgress(targetId, progress)).then((items) => items[0])
                                : await requestGeneration({ ...generationConfig, count: "1" }, effectivePrompt, (progress) => updateGenerationProgress(targetId, progress)).then((items) => items[0]);
                            const uploaded = await storeGeneratedImage(image);
                            ...
                            setNodes((prev) =>
                                prev.map((node) => {
                                    ...
                                    return {
                                        ...
                                        metadata: { ...node.metadata, ...imageMetadata(uploaded), progress: undefined, primaryImageId: targetId === rootId ? rootId : node.metadata?.primaryImageId },
                                    };
                                }),
                            );
                            hasSuccess = true;
                            if (isConfigNode) setNodes((prev) => prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_SUCCESS, progress: undefined, errorDetails: undefined } } : node)));
                            return true;
                        } catch (error) {
                            const errorDetails = error instanceof Error ? error.message : "生成失败";
                            hasFailure = true;
                            setNodes((prev) => prev.map((node) => (node.id === targetId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, progress: undefined, errorDetails } } : node)));
                            return false;
                        }
                    });
                    ...
                    setNodes((prev) =>
                        prev.map((node) =>
                            node.id === nodeId && isConfigNode
                                ? { ...node, metadata: { ...node.metadata, status: hasSuccess ? NODE_STATUS_SUCCESS : NODE_STATUS_ERROR, progress: undefined, errorDetails: hasSuccess ? undefined : "全部图片生成失败" } }
                                : node,
                        ),
                    );
                    return;
                ...
                if (mode === "video") {
                    ...
                    const videoNode: CanvasNodeData = {
                        ...
                        metadata: { prompt: effectivePrompt, status: NODE_STATUS_LOADING, progress: undefined, model: generationConfig.model, size: generationConfig.videoSize, videoSize: generationConfig.videoSize, seconds: generationConfig.videoSeconds, vquality: generationConfig.vquality, generateAudio: generationConfig.videoGenerateAudio, watermark: generationConfig.videoWatermark, references: generationReferenceUrls(generationContext) },
                    };
                    pendingChildIds = [videoId];
                    setNodes((prev) => (isEmptyVideoNode ? prev.map((node) => (node.id === nodeId ? { ...node, ...videoNode } : node)) : [...prev.map((node) => (node.id === nodeId ? { ...node, metadata: { ...node.metadata, status: NODE_STATUS_SUCCESS, progress: undefined } } : node)), videoNode]));
                    if (!isEmptyVideoNode) setConnections((prev) => [...prev, { id: nanoid(), fromNodeId: nodeId, toNodeId: videoId }]);
                    const video = await storeGeneratedVideo(await requestVideoGeneration(generationConfig, effectivePrompt, generationContext.referenceImages, generationContext.referenceVideos, generationContext.referenceAudios, (progress) => updateGenerationProgress(videoId, progress)));
                    const videoSize = fitNodeSize(video.width || spec.width, video.height || spec.height, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
                    setNodes((prev) => prev.map((node) => (node.id === videoId ? { ...node, width: videoSize.width, height: videoSize.height, position: { x: node.position.x + node.width / 2 - videoSize.width / 2, y: node.position.y + node.height / 2 - videoSize.height / 2 }, metadata: { ...node.metadata, ...videoMetadata(video), progress: undefined, prompt: effectivePrompt, model: generationConfig.model, size: generationConfig.videoSize, videoSize: generationConfig.videoSize, seconds: generationConfig.videoSeconds, vquality: generationConfig.vquality, generateAudio: generationConfig.videoGenerateAudio, watermark: generationConfig.videoWatermark, references: generationReferenceUrls(generationContext) } } : node)));
                    return;
                }
                ...
                if (mode === "audio") {
                    ...
                    const audioNode: CanvasNodeData = {
                        ...
                        metadata: { prompt: effectivePrompt, status: NODE_STATUS_LOADING, progress: undefined, ...buildAudioGenerationMetadata(generationConfig) },
                    };
                    ...
                    setNodes((prev) => prev.map((node) => (node.id === audioId ? { ...node, metadata: { ...node.metadata, ...audioMetadata(audio), progress: undefined, prompt: effectivePrompt, ...buildAudioGenerationMetadata(generationConfig) } } : node)));
                    return;
                }
                ...
                message.error(errorDetails);
                setNodes((prev) =>
                    prev.map((node) => (node.id === nodeId || pendingChildIds.includes(node.id) ? (node.id === nodeId && !markSourceStatus ? node : { ...node, metadata: { ...node.metadata, status: NODE_STATUS_ERROR, progress: undefined, errorDetails } }) : node)),
                );
            } finally {
                setRunningNodeId(null);
            }
        ...
        async (node: CanvasNodeData) => {
            ...
            const generationConfig =
                hasSavedImageMetadata && savedImageMetadata
                    ? {
                          ...effectiveConfig,
                          model: savedImageMetadata.model && isModelAvailableForCapability(effectiveConfig, savedImageMetadata.model, "image") ? savedImageMetadata.model : effectiveConfig.imageModel || effectiveConfig.model,
            ...
            const updateRetryProgress = (progress: number) => {
                setNodes((prev) => prev.map((item) => (item.id === node.id && item.metadata?.status === NODE_STATUS_LOADING ? { ...item, metadata: { ...item.metadata, progress } } : item)));
            };

            setRunningNodeId(node.id);
            setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_LOADING, progress: undefined, errorDetails: undefined } } : item)));
            ...
            try {
                ...
                if (node.type === CanvasNodeType.Video) {
                    const video = await storeGeneratedVideo(await requestVideoGeneration(generationConfig, prompt, retryImages, context?.referenceVideos || [], context?.referenceAudios || [], updateRetryProgress));
                    const videoSize = fitNodeSize(video.width || node.width, video.height || node.height, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
                    setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, width: videoSize.width, height: videoSize.height, position: { x: item.position.x + item.width / 2 - videoSize.width / 2, y: item.position.y + item.height / 2 - videoSize.height / 2 }, metadata: { ...item.metadata, ...videoMetadata(video), progress: undefined, prompt, model: generationConfig.model, size: generationConfig.videoSize, videoSize: generationConfig.videoSize, seconds: generationConfig.videoSeconds, vquality: generationConfig.vquality, generateAudio: generationConfig.videoGenerateAudio, watermark: generationConfig.videoWatermark } } : item)));
                    return;
                }
                if (node.type === CanvasNodeType.Audio) {
                    ...
                    setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, ...audioMetadata(audio), progress: undefined, prompt, ...buildAudioGenerationMetadata(generationConfig) } } : item)));
                    return;
                }

                const image = useReferenceImages ? await requestEdit(generationConfig, prompt, retryImages, undefined, updateRetryProgress).then((items) => items[0]) : await requestGeneration(generationConfig, prompt, updateRetryProgress).then((items) => items[0]);
                    ...
                    prev.map((item) =>
                        item.id === node.id
                            ? {
                                  ...
                                  width: imageSize.width,
                                  height: imageSize.height,
                                  metadata: { ...item.metadata, ...imageMetadata(uploadedImage), progress: undefined, prompt, ...generationMetadata },
                              }
                            : item,
                    ),
                );
                ...
                const errorDetails = error instanceof Error ? error.message : "生成失败";
                message.error(errorDetails);
                setNodes((prev) => prev.map((item) => (item.id === node.id ? { ...item, metadata: { ...item.metadata, status: NODE_STATUS_ERROR, progress: undefined, errorDetails } } : item)));
            } finally {
                setRunningNodeId(null);
            }
        },
    ...
    const generateImageFromTextNode = useCallback(
        ...
        [effectiveConfig.canvasImageCount, effectiveConfig.count, effectiveConfig.imageModel, effectiveConfig.model, effectiveConfig.size, effectiveConfig.videoSize, message],
    );
...
function getImageBatchDisplayCount(node: CanvasNodeData) {
    if (node.type !== CanvasNodeType.Image || !node.metadata?.isBatchRoot) return 0;
    const childCount = node.metadata.batchChildIds?.length || 0;
    return Math.max(node.metadata.count || 0, childCount + 1);
}
...
function generationModelForMode(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode) {
    const defaultModel = mode === "image" ? config.imageModel : mode === "video" ? config.videoModel : mode === "audio" ? config.audioModel : config.textModel;
    const savedModel = node?.metadata?.model;
    return savedModel && isModelAvailableForCapability(config, savedModel, mode) ? savedModel : defaultModel || (mode === "audio" ? defaultConfig.audioModel : config.model || defaultConfig.model);
}

function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode): AiConfig {
    return {
        ...config,
        model: generationModelForMode(config, node, mode),
        quality: node?.metadata?.quality || config.quality || defaultConfig.quality,
        size: mode === "image" ? node?.metadata?.size || config.size || defaultConfig.size : config.size,
        videoSize: mode === "video" ? node?.metadata?.videoSize || (node?.type === CanvasNodeType.Video ? node.metadata?.size : undefined) || config.videoSize || defaultConfig.videoSize : config.videoSize,
